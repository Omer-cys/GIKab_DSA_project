...
void showCircularRides(int riderId) {
    ...
    while (current != NULL) {
        cout << "Ride " << current->rideId << ": " << current->pickup
             << " -> " << current->dropoff << " | Fare: " << current->fare << endl;
        current = current->next;
        if (current == NULL) current = start; // repeat once
...
void splitRideList(Ride* source, Ride** front, Ride** back) {
...
void showRidesSortedByFare(int riderId) {
    int index = riderId - 100;
    if (index < 0 || index >= userCount || rideHistory[index].head == NULL) {
        cout << "No rides found." << endl;
        return;
    ...
    rideHistory[index].head = mergeSortRides(rideHistory[index].head);

    // reset tail
    Ride* cur = rideHistory[index].head;
    rideHistory[index].tail = NULL;
    while (cur != NULL) {
        if (cur->next == NULL) {
            rideHistory[index].tail = cur;
        }
        cur = cur->next;
    }

    cout << "--- Rides Sorted by Fare ---" << endl;
    cur = rideHistory[index].head;
    while (cur != NULL) {
        cout << "Ride " << cur->rideId << ": " << cur->pickup
             << " -> " << cur->dropoff << " | Fare: " << cur->fare << endl;
        cur = cur->next;
    }
}
...
void dequeueRideRequest() {
    ...
    if (driver != NULL && driver->isDriver) {
        cout << "Ride assigned to Driver " << driver->name 
             << " (ID: " << driver->id << ") for Rider " 
             << temp->riderId << endl;

        // Optional: ask for rating immediately
        double rating;
...
void inorderDriverTraversal(DriverNode* root) {
    ...
    cout << root->driver->name << " (ID: " << root->driver->id
         << ") | Rating: " << root->driver->rating << endl;
...
int main() {
    ...
    while (choice != 0) {
        ...
        if (choice == 1) {
            ...
            if (u != NULL) cout << "Found: " << u->name << " (Role: " 
